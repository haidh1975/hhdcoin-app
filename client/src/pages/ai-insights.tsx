...
export default function AIInsights() {
  ...
  const { data: marketAnalysis, isLoading: isLoadingAnalysis, refetch: refetchAnalysis } = useQuery<MarketAnalysis>({
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold capitalize">{marketAnalysis.trend}</span>
                    </div>
                    <Badge variant="secondary" data-testid="badge-confidence">
                      Độ tin cậy: {Math.round(marketAnalysis.confidenceScore * 100)}%
                    </Badge>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={getRiskColor(marketAnalysis.riskLevel)} data-testid="badge-risk-level">
                      {marketAnalysis.riskLevel}
                    </Badge>
                  </div>

                  {/* Price Targets */}
                  {marketAnalysis.priceTarget && (
                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-semibold">${marketAnalysis.priceTarget.short_term?.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-gray-600 dark:text-gray-400">Trung hạn</p>
                          <p className="font-semibold">${marketAnalysis.priceTarget.medium_term?.toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                    ...
                    <p className="text-sm">{marketAnalysis.recommendation}</p>
                  </div>

                  <div className="text-xs text-gray-500 mt-4">
                    Loại phân tích: {marketAnalysis.analysisType} | 
                    Cập nhật: {new Date(marketAnalysis.timestamp).toLocaleString('vi-VN')}
                  </div>
                </div>
              ) : (
